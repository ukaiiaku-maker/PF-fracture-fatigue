...
def compare(rows, returncode):
    ...
    passed = (returncode == 1 and set(failures) == set(inherited)
              and all(r["match"] for r in comparison)
...
def run(phase):
    ...
    if phase == "full":
        ...
        assert focus["relative_gate_pass"], "focused relative gate has not passed"
        assert code_hashes() == focus["source_file_hashes"], "source changed after focused validation"
