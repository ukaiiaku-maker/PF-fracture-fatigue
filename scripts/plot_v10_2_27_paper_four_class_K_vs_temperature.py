#!/usr/bin/env python3
"""Temperature summaries for the v10.2.27 four-class 1000 um R-curves.

For every complete case, report four resistance measures:

* initial: first accepted cleavage-event resistance;
* intermediate: resistance at one-half of the requested projected extension;
* end: resistance at the requested projected extension;
* average: crack-extension-weighted mean resistance over the requested interval.

The interpolation and integration use the same realized discrete R-curve semantics
as the campaign R-curve postprocessor: resistance is constant through each accepted
event interval and linearly interpolated only across gaps between event intervals.
"""
...
def extension_weighted_average(
    ...
    """Integrate the realized R-curve over crack extension, then divide by target."""
    target = float(target_um)
    ...
    y = np.asarray(
        [resistance_at_extension(pre, post, resistance, value, achieved_um) for value in x],
        dtype=float,
    )
...
def main() -> int:
    ...
    for option_dir in sorted(path for path in outroot.iterdir() if path.is_dir()):
        ...
        for case in sorted(path for path in option_dir.iterdir() if path.is_dir()):
            ...
            achieved_um = float(np.max(achieved_values)) if achieved_values.size else float("nan")
            initial = float(resistance[0])
            intermediate = resistance_at_extension(
                pre, post, resistance, intermediate_um, achieved_um
            )
            end = resistance_at_extension(pre, post, resistance, target_um, achieved_um)
            average = extension_weighted_average(
            ...
            records.append(
                {
                    ...
                    "K_initial_MPa_sqrt_m": initial,
                    "K_intermediate_MPa_sqrt_m": intermediate,
                    "K_end_MPa_sqrt_m": end,
                    "K_average_MPa_sqrt_m": average,
                    "average_definition": (
    ...
    records.sort(key=lambda row: (rank.get(row["option_key"], 999), row["temperature_K"]))
    ...
    for option in options:
        ...
        ax.set_title(f"{SHORT_LABELS.get(option, option)}: resistance metrics versus T")
        ax.set_ylim(bottom=0.0)
        ax.legend(frameon=True)
        _save(fig, plot_dir / "by_candidate" / f"{option}_K_metrics_vs_temperature.png")
