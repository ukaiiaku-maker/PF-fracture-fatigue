"""Build/extend a portable, tracked event ledger for the static-shield
attribution study, in the same style as scripts/build_developed_
confirmation_event_ledger.py -- so downstream comparison and verification
never depend on gitignored runs/ directories.

Usage:
    <pinned interpreter> scripts/build_static_shield_attribution_event_ledger.py \\
        --run-root runs/crack_rebonding_static_shield_attribution/stage1_seed1720
"""
...
ARTIFACTS_DIR = REPO_ROOT / "artifacts/crack_rebonding_static_shield_attribution"
...
EVENT_FIELDS = [
    ...
    "any_bulk_action_used", "all_bulk_action_qualified", "K_b_applied_Pa_sqrt_m",
]
MPZ_STATE_FIELDS = [
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ledger_path.is_file():
        ...
        ledger = {
            "schema": "v10.2.30_crack_rebonding_static_shield_attribution_event_ledger_v1",
    ...
    for name, res in trajectories.items():
        if name in ledger["trajectories"]:
            raise SystemExit(
                ...
                "no restart/resume or silent overwrite authorized"
            )
        events_out = []
        for e in res["events"]:
        ...
        ledger["trajectories"][name] = {
            ...
            "seed": res["seed"],
            "Kmax_Pa_sqrt_m": res["Kmax_Pa_sqrt_m"],
            "T_K": res["T_K"],
            ...
            "no_resume_no_restart_marker": res.get("no_resume_no_restart_marker"),
            "rng_state_or_stream_identifier": res.get("rng_state_or_stream_identifier"),
            "frozen_configuration_sha256": res.get("frozen_configuration_sha256"),
            "git_head_at_launch": res.get("git_head_at_launch"),
            "producer_file_sha256_at_launch": res.get("producer_file_sha256_at_launch"),
            "events": events_out,
