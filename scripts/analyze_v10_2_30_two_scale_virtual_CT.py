...
def integrate_PT_overlays(root:Path,native_surface:LogPchipRateSurface,native_curves:pd.DataFrame,native_summary:pd.DataFrame):
    ...
    results_frame=pd.DataFrame(results);validation_frame=pd.DataFrame(validation)
    validation_frame["refinement_required"]=validation_frame.stage=="PT_LOW_K_13P5"
    validation_frame["refinement_resolved"]=False
    for index,row in validation_frame[validation_frame.refinement_required].iterrows():
        match=validation_frame[(validation_frame.stage=="PT_LOW_K_REFINEMENT")&(validation_frame.option==row.option)&np.isclose(validation_frame.R,row.R)]
        validation_frame.loc[index,"refinement_resolved"]=bool(len(match)==1 and match.admissible.iloc[0])
    validation_frame.loc[~validation_frame.refinement_required,"refinement_resolved"]=True
    results_frame.to_csv(root/"PT_conditional_anchor_results.csv",index=False);validation_frame.to_csv(root/"PT_conditional_anchor_validation.csv",index=False)
    unresolved=validation_frame[~validation_frame.admissible & ~validation_frame.refinement_resolved]
    if len(unresolved):raise RuntimeError("conditional PT anchor error exceeds 0.05 decade without matching refinement")
    surfaces={};ratio_rows=[]
    for option,filename in ((PT03,"PT03_rate_surface.json"),(PT08,"PT08_rate_surface.json")):
