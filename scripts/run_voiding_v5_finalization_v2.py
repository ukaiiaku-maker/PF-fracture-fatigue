...
from arrhenius_fracture.voiding_production_v5 import (
    ...
    downstream_front_transaction, ligament_transaction, natural_trajectory,
    observables,
)
from arrhenius_fracture.voiding_v5 import (
    Cavity2D, ProductionVoidState, VoidPhase, VoidingConfig, advance_site,
    arrhenius_rates, grow_cavity_from_rate, promote_cavity,
)
...
def restart_rows(out):
    sources={}; initial,_=build_production_void_state(stochastic=True,seed=9100); sources["available_site"]=initial
    pre,_=deterministic_trajectory(stop_before_ligament=True); sources["before_ligament"]=pre
    connected,_=ligament_transaction(pre); sources["connected_before_downstream"]=connected
    child,result,_,_=downstream_front_transaction(connected)
    if result is not None: sources["downstream_front_before_continuation"]=child
    rows=[]
    for index,stage in enumerate(RESTART_STAGES):
        ...
        same=complete_accepted_state_fingerprint(state)==complete_accepted_state_fingerprint(restored)
        rows.append({"execution_id":f"restart-{index:02d}","stage":stage,"uninterrupted_fingerprint":complete_accepted_state_fingerprint(state),
          "restored_fingerprint":complete_accepted_state_fingerprint(restored),"passed":same,
          "classification":"SERIALIZATION_EXACT; CONTINUATION_NOT_EXECUTED"})
    return rows
...
def main(argv=None):
    ...
    parser.add_argument("--implementation-sha",default=os.environ.get("VOIDING_V5_IMPLEMENTATION_SHA")); args=parser.parse_args(argv)
    out=Path(args.out); out.mkdir(parents=True,exist_ok=True); head=subprocess.check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip()
    ...
    neutrality=[{"execution_id":f"neutrality-{name}","trajectory":name,"passed":False,
      "classification":"CLEAN_STAGE_II_BASE_WORKTREE_COMPARISON_NOT_EXECUTED"} for name in ("monotonic","oblique","checkpoint_restart","unload_reload")]
    all_execution_ids=[x["execution_id"] for rows in (transitions,controlled,restarts,natural,rollback,neutrality) for x in rows]
    gates={"TRANSITION_PARTITIONS":all(x["passed"] and x["partition_equivalent"] for x in transitions),
      ...
      "LENGTH_INVENTORY_CONSERVATION":False}
