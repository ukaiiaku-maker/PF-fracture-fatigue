...
case "$MODE" in
  smoke)
    ...
    ;;
  pilot)
    DELTA_KS_MPA=${DELTA_KS_MPA:-"15 18 21 24 27"}
...
OUTROOT=${OUTROOT:-runs/v10_2_1_fixed_deltaK_${MODE}_${CLASS}_${TEMP_K}K_th${THETA}_${GIT_SHORT}}
if [[ "$FORCE" == 1 ]]; then rm -rf "$OUTROOT"; fi
mkdir -p "$OUTROOT"

printf 'v10.2.1 fixed-DeltaK sweep\n'
...
for DELTA_K in $DELTA_KS_MPA; do
  ...
  for SEED in $SEEDS; do
    ...
    "$PYTHON_BIN" -u -m arrhenius_fracture.sharp_front_v10_2_1 \
      --mode 2d \
      --fatigue-cycles \
...
audit = json.loads((root / "v10_2_1_fixed_deltaK_control.json").read_text())
assert math.isclose(float(audit["target_deltaK_MPa_sqrt_m"]), target, rel_tol=0, abs_tol=1e-12)
assert audit["fatigue_control_mode"] == "prescribed_fixed_local_deltaK"
assert audit["fixed_deltaK_exact_within_relative_1e-12"] is True, audit
assert int(audit["waveforms_created"]) > 0, audit
print(json.dumps({
    ...
    "events": int(audit["stochastic_geometry_events"]),
    "status": audit["censor_status"],
    "maximum_abs_deltaK_error_Pa_sqrt_m": audit["maximum_abs_target_error_Pa_sqrt_m"],
}, indent=2))
PY
  done
done
...
printf '\nv10.2.1 fixed-DeltaK sweep complete: %s\n' "$OUTROOT"
