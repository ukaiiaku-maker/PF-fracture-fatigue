#!/usr/bin/env python3
"""Compare v9.13 1-D K50 predictions with v10.2.23 stochastic 2-D results."""
...
def main() -> int:
    ...
    for row in one_rows:
        ...
        for temperature, column in reference_columns:
            ...
            merged.append(
                {
                    "search_rank": int(round(_number(row["search_rank"]))),
                    ...
                    "y__peak_temperature_K": _number(row["y__peak_temperature_K"]),
                    "y__peak_prominence": _number(row["y__peak_prominence"]),
                }
            )

    args.out_dir.mkdir(parents=True, exist_ok=True)
    _write_csv(args.out_dir / "v10_2_23_top10_1d_2d_K50_comparison.csv", merged)
    ...
    for candidate, local in by_candidate.items():
        ...
        summaries.append(
            {
                "search_rank": int(local[0]["search_rank"]),
                "candidate_id": candidate,
    ...
    summaries.sort(key=lambda row: int(row["search_rank"]))
    _write_csv(args.out_dir / "v10_2_23_top10_1d_2d_candidate_errors.csv", summaries)
    print(
        "V10223_1D_2D_COMPARISON "
