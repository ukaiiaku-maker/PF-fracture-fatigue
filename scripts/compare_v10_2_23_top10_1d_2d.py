            ...
            records.append(
                {
                    ...
                    "y__peak_temperature_K": float(row["y__peak_temperature_K"]),
                    "y__peak_prominence": float(row["y__peak_prominence"]),
                }
            )
...
def main() -> int:
    ...
    merged.to_csv(args.out_dir / "v10_2_23_top10_1d_2d_K50_comparison.csv", index=False)
    ...
    for candidate, local in merged.groupby("candidate_id", sort=False):
        ...
        summaries.append(
            {
                "search_rank": int(local["search_rank"].iloc[0]),
    ...
    summary = pd.DataFrame(summaries).sort_values("search_rank")
    summary.to_csv(args.out_dir / "v10_2_23_top10_1d_2d_candidate_errors.csv", index=False)
    print(
