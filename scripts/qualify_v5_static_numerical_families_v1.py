...
from arrhenius_fracture.static_numerical_family_v2 import SENTINEL_FAMILIES
...
def main():
    ...
    parser.add_argument('--sentinel-v2',action='store_true')
    parser.add_argument('--shard-index',type=int,default=0)
    parser.add_argument('--shard-count',type=int,default=1)
    args = parser.parse_args()
    if not 0<=args.shard_index<args.shard_count:raise ValueError('invalid static shard')
    if args.sentinel and args.sentinel_v2:raise ValueError('select one sentinel protocol')
    if (args.sentinel or args.sentinel_v2) and args.shard_count!=1:raise ValueError('sentinel cannot be sharded')
    if args.output.exists(): raise ValueError('refusing to overwrite evidence')
    status = subprocess.check_output(
        ('git','status','--porcelain','--untracked-files=no'),cwd=ROOT,text=True)
    if status.strip() and not args.sentinel: raise ValueError('static qualification requires clean committed code')
    sha = subprocess.check_output(('git','rev-parse','HEAD'),cwd=ROOT,text=True).strip()
    groups = ({'centered:{mesh}':GROUPS['centered:{mesh}']} if args.sentinel else
              {name:GROUPS[name] for name in SENTINEL_FAMILIES} if args.sentinel_v2 else GROUPS)
    ...
    result={'schema':SCHEMA,'executed_code_sha':sha,'worktree_status':status,
        'sentinel_only':args.sentinel or args.sentinel_v2,
        'sentinel_protocol':'STATIC_FAMILY_V2' if args.sentinel_v2 else 'LEGACY_CENTERED_V1' if args.sentinel else None,
