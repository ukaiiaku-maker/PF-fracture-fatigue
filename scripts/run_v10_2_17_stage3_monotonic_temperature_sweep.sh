...
if [[ -z "$SIGNED_KERNEL_FAMILY_JSON" || ! -f "$SIGNED_KERNEL_FAMILY_JSON" ]]; then
  echo "ERROR: set SIGNED_KERNEL_FAMILY_JSON to the campaign-ready v10.2.14 family" >&2
  exit 2
fi
...
if ! [[ "$BASE_HAZARD_SEED" =~ ^[0-9]+$ ]]; then
  echo "ERROR: BASE_HAZARD_SEED must be a nonnegative integer" >&2
  exit 2
fi
...
SIGNED_KERNEL_FAMILY_JSON="$SIGNED_KERNEL_FAMILY_JSON" "$PYTHON_BIN" - <<'PY'
import os
...
if family.metadata.get("production_parameterization_allowed") is not True:
    raise SystemExit("signed family is not production-authorized")
...
assert entry.LOW_LEVEL_DRIVER.endswith("sharp_front_v10_1_7_5")
print(
    f"project={arrhenius_fracture.PROJECT_ID} "
    f"release={arrhenius_fracture.PROJECT_RELEASE} package={actual} "
    f"signed_states={len(family.states)} entry={entry.MODEL_ID}"
)
PY
...
base_seed = int(os.environ["BASE_HAZARD_SEED"])
...
if not allow_partial:
    if tuple(options) != CANONICAL_STAGE3_OPTIONS:
        raise SystemExit(
            f"full Stage 3 requires options {CANONICAL_STAGE3_OPTIONS}; got {tuple(options)}"
        )
    expected_temps = [float(value) for value in range(300, 1201, 100)]
    if temps != expected_temps:
        raise SystemExit(
            f"full Stage 3 requires temperatures {expected_temps}; got {temps}"
        )
print(
    "option_key\tcandidate_id\ttemperature_K\tmpz_length_um\t"
    "mpz_n_bins\thazard_seed\tcase_root"
)
for option_index, option in enumerate(options):
    selected = select_option(option, registry, canonical_stage3_only=True)
    for temperature in temps:
        seed = base_seed + option_index * 10000 + int(round(temperature))
        tag = f"T{int(round(temperature))}K_th{theta:g}_seed{seed}"
        case_root = outroot / selected.option_key / tag
        print(
            f"{selected.option_key}\t{selected.candidate_id}\t{temperature:g}\t"
...
cat <<EOF
Stage 3 final signed stochastic parameter-overlay plan
  entry:         arrhenius_fracture.sharp_front_v10_2_17
  signed engine: state_resolved_signed_engine_v10214
  kernel family: $SIGNED_KERNEL_FAMILY_JSON
  hazard:        exponential
  event length:  threshold_scaled [$CLEAVAGE_EVENT_MIN_FACTOR,$CLEAVAGE_EVENT_MAX_FACTOR]
  mode:          $MODE
  cases:         $N_CASES
  options:       $OPTIONS
  temperatures:  $TEMPS
...
run_case() {
  ...
  mkdir -p "$case_root"
  if [[ "$SKIP_FINISHED" == "1" \
        && -f "$case_root/stage3_case_status.json" \
        && -f "$case_root/v10_2_17_final_signed_stochastic_stack.json" ]]; then
    echo "SKIP finished: $option T=${temperature}K seed=$hazard_seed"
    return 0
  fi
  rm -f "$case_root/RUN_FAILED" "$case_root/exit_code.txt"

  local cmd=(
    "$PYTHON_BIN" -u -m arrhenius_fracture.sharp_front_v10_2_17
...

  {
    ...
    printf 'ANISOTROPIC_USE_AVALANCHE_BACKEND=1 PARAMETER_CAMPAIGN=1 '
    printf '%q ' "${cmd[@]}"
    printf '\n'
  } > "$command_file"
  chmod +x "$command_file"

  echo "START: $option candidate=$candidate T=${temperature}K seed=$hazard_seed mpz=${mpz_length}um/${mpz_bins}bins"
  env \
...
while IFS=$'\t' read -r option candidate temperature mpz_length mpz_bins hazard_seed case_root <&3; do
  [[ -n "$option" ]] || continue
  wait_for_slot
  run_case "$option" "$candidate" "$temperature" "$mpz_length" "$mpz_bins" "$hazard_seed" "$case_root" &
  PIDS+=("$!")
  LABELS+=("$option/T${temperature}K/seed${hazard_seed}")
done
exec 3<&-
