...
def suite_result():
    ...
    return dict(result,recorded_testcases=len(nodes),failures=failures,
        skipped=sum(c.find('skipped') is not None for c in nodes),
        merge_gate='TEST_SUITE_PASS' if result['returncode']==0 else 'NOT_CLEARED_FULL_SUITE_FAILURES')
