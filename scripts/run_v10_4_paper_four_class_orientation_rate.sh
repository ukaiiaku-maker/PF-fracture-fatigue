...
# Corrected v10.4.3 production must start in a fresh output root.  The old
# v10.4.1 cases advanced plasticity once per stagger iteration and are not
# physically reusable after the constitutive-time correction.
if [[ "${ALLOW_V1041_REUSE_AFTER_STAGGER_FIX:-0}" == 1 ]]; then
  echo "WARNING: legacy reuse is enabled for audit-only verification" >&2
else
  export ALLOW_V1041_REUSE_AFTER_STAGGER_FIX=0
fi
...
GENERATED_BASE=$(mktemp "$ROOT/scripts/.v10_4_3_stagger_consistent.XXXXXX")
...
printf 'Stagger semantics: under-relaxed fixed point over one accepted dt\n'
printf 'Unconverged trial: exact rollback, then smaller dt and dU at fixed loading rate\n'
printf 'Minimum-dt failure: hard stop before physical-step acceptance\n'
printf 'Accepted plastic work: converged constitutive state only\n'
printf 'Plastic-flow terminal: enabled for controlled validation\n'
...
printf 'Legacy v10.4.1 reuse: disabled for corrected production\n'
...
if [[ "${PREFLIGHT_ONLY:-0}" != 1 ]]; then
  : "${OUTROOT:?OUTROOT must be set for v10.4.3 production plotting}"
