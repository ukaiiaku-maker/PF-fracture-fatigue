...
print(payload["theta_deg"], payload["branching_mode"], payload["maximum_fronts"])
PY
)
...
if [[ -z "$SNAPSHOT_ROOT" && -z "$SNAPSHOT_ARCHIVE" ]]; then
  : "${KERNEL_CAPTURE_COMMAND:?Set KERNEL_CAPTURE_COMMAND to generate snapshots for this mechanical configuration}"
  ...
  export V10227_KERNEL_CAPTURE_REQUIRED_MAX_EXTENSION_UM="${V10227_KERNEL_REQUIRED_MAX_EXTENSION_UM:-$TARGET_EXT_UM}"
  echo "CAPTURE kernel states: $KERNEL_CAPTURE_COMMAND" >&2
  bash -lc "$KERNEL_CAPTURE_COMMAND"
fi

if [[ -z "$LOAD_ROOT" && -z "$LOAD_ARCHIVE" ]]; then
  if [[ -z "$SNAPSHOT_ROOT" ]]; then
    echo "ERROR: automatic load-invariance evaluation requires an extracted snapshot root" >&2
    exit 2
  fi
  ...
  while IFS= read -r snapshot_json; do
    ...
    if [[ -f "$destination/frozen_geometry_load_invariance.json" ]]; then
      echo "REUSE load invariance: $state" >&2
      continue
    fi
  ...
  if [[ "$found" -lt 2 ]]; then
    echo "ERROR: capture command produced fewer than two frozen states in $SNAPSHOT_ROOT" >&2
    exit 2
  fi
fi
