#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
REPO_ROOT=$(cd -- "$SCRIPT_DIR/.." && pwd)
cd "$REPO_ROOT"
export PYTHONPATH="$REPO_ROOT${PYTHONPATH:+:$PYTHONPATH}"

PYTHON_BIN=${PYTHON_BIN:-python}
TEMPS=${TEMPS:-"300 1100"}
GEOMETRY_GAINS=${GEOMETRY_GAINS:-"0 1 4 9"}
TARGET_EXT_UM=${TARGET_EXT_UM:-100}
STEPS=${STEPS:-4000}
PRINT_EVERY=${PRINT_EVERY:-100}
OUTROOT=${OUTROOT:-runs/v10_1_9_1_geometry_source_feedback_DBTT_100um_v1}
FORCE=${FORCE:-0}

NX=${NX:-48}
NY=${NY:-96}
TIP_H_FINE=${TIP_H_FINE:-5e-7}
TIP_RATIO=${TIP_RATIO:-1.2}
DU=${DU:-2e-7}
DT=${DT:-8.4}
N_STAGGER=${N_STAGGER:-2}
DA_CHECKPOINT_M=${DA_CHECKPOINT_M:-5e-6}
MPZ_LENGTH_UM=${MPZ_LENGTH_UM:-100}
MPZ_N_BINS=${MPZ_N_BINS:-200}
WAKE_LENGTH_UM=${WAKE_LENGTH_UM:-100}
WAKE_N_BINS=${WAKE_N_BINS:-0}
THETA=${THETA:-45}
EVENT_TARGET=${EVENT_TARGET:-0.05}
SAVE_SNAPSHOTS=${SAVE_SNAPSHOTS:-0}
SNAPSHOT_BY_EXT_UM=${SNAPSHOT_BY_EXT_UM:-5}
PACKET_LENGTH_M=${PACKET_LENGTH_M:-2.5e-10}
MOBILE_SHIELD_FRACTION=${MOBILE_SHIELD_FRACTION:-1.0}
KINETIC_MAX_ACTION_SUBSTEP=${KINETIC_MAX_ACTION_SUBSTEP:-0.01}
KINETIC_MAX_TRANSLATION_SUBSTEP_M=${KINETIC_MAX_TRANSLATION_SUBSTEP_M:-5e-8}
K_FIRST_MAX_MPA_SQRT_M=${K_FIRST_MAX_MPA_SQRT_M:-200}

mkdir -p "$OUTROOT"
MANIFEST="$OUTROOT/geometry_source_manifest.tsv"
printf 'kind\tgeometry_gain\ttemperature_K\tstatus\toutdir\n' > "$MANIFEST"

scale_tag() {
  printf '%s' "$1" | sed 's/-/m/g; s/\./p/g'
}

run_case() {
  local kind=$1
  local gain=$2
  local temp=$3
  local outdir=$4
  shift 4
  local flags=("$@")

  mkdir -p "$outdir"
  echo "========================================================================"
  echo "v10.1.9.1 geometry source feedback: kind=$kind T=${temp}K gain=$gain"
  echo "target=${TARGET_EXT_UM}um out=$outdir"
  echo "========================================================================"

  local status=COMPLETE
  if [[ "$FORCE" != 1 && -s "$outdir/summary.json" && -s "$outdir/kinetic_tip_cell_audit_v101.json" ]]; then
    echo "SKIP existing complete-looking case: $outdir"
    status=EXISTING
  else
    if ! TIP_GEOMETRY_SOURCE_GAIN="$gain" \
         CAMPAIGN_BACKSTRESS_SCALE=1 \
         CAMPAIGN_REFRESH_SCALE=1 \
         "$PYTHON_BIN" -m arrhenius_fracture.sharp_front_v10_1_9 \
      --mode 2d --material-class DBTT --temperatures "$temp" \
      --bulk-plasticity-mode tip_only --directional-j-mode root_signed \
      --tip-kinetics-mode moving_velocity --tip-source-model continuum \
      "${flags[@]}" \
      --mobile-shield-fraction "$MOBILE_SHIELD_FRACTION" \
      --kinetic-packet-length-m "$PACKET_LENGTH_M" \
      --kinetic-max-action-substep "$KINETIC_MAX_ACTION_SUBSTEP" \
      --kinetic-max-translation-substep-m "$KINETIC_MAX_TRANSLATION_SUBSTEP_M" \
      --steps "$STEPS" --nx "$NX" --ny "$NY" \
      --dU "$DU" --dt "$DT" --n-stagger "$N_STAGGER" \
      --tip-h-fine "$TIP_H_FINE" --tip-ratio "$TIP_RATIO" \
      --da-phys "$DA_CHECKPOINT_M" --target-crack-extension-um "$TARGET_EXT_UM" \
      --mpz-length-um "$MPZ_LENGTH_UM" --mpz-n-bins "$MPZ_N_BINS" \
      --wake-length-um "$WAKE_LENGTH_UM" --wake-n-bins "$WAKE_N_BINS" \
      --no-wake-shielding \
      --crystal-aniso --crystal-compete --crystal-theta-deg "$THETA" \
      --crystal-material w --j-decomposition cluster \
      --max-fronts 1 --adaptive-events --adaptive-event-target "$EVENT_TARGET" \
      --print-every "$PRINT_EVERY" --save-snapshots "$SAVE_SNAPSHOTS" \
      --snapshot-by-crack-extension-um "$SNAPSHOT_BY_EXT_UM" \
      --out "$outdir"; then
      status=FAILED
    fi
  fi

  if [[ "$status" != FAILED ]]; then
    if ! "$PYTHON_BIN" - "$outdir" "$kind" "$gain" "$TARGET_EXT_UM" \
      "$DA_CHECKPOINT_M" "$K_FIRST_MAX_MPA_SQRT_M" <<'PY'
import json, math, pathlib, sys

root = pathlib.Path(sys.argv[1])
kind = sys.argv[2]
gain = float(sys.argv[3])
target_um = float(sys.argv[4])
da_m = float(sys.argv[5])
kmax = float(sys.argv[6])

modes = json.loads((root / "v10_1_driver_modes.json").read_text())
assert modes["geometry_source_feedback"] is True, modes
assert modes["first_passage_feedback_disabled"] is True, modes
assert modes["geometry_feedback_armed_only_after_first_advance"] is True, modes
assert modes["geometry_running_maximum_blunting"] is True, modes
assert modes["geometry_reference"] == "original unblunted radius r0", modes
assert modes["wake_shielding"] is False, modes
assert math.isclose(float(modes["geometry_source_gain"]), gain, abs_tol=1e-14)

summary = json.loads((root / "summary.json").read_text())
row = summary[0]
minimum_advances = max(1, math.ceil(target_um / (da_m * 1.0e6) - 1.0e-12))
assert int(row["n_advances"]) >= minimum_advances, row
kc = float(row["Kc_first_MPa_sqrt_m"])
assert math.isfinite(kc) and 0.0 < kc <= kmax, row

audit = json.loads((root / "kinetic_tip_cell_audit_v101.json").read_text())
records = audit.get("records", [])
assert records, audit
required = (
    "geometry_source_gain",
    "geometry_source_feedback_armed",
    "geometry_source_r0_m",
    "geometry_source_current_radius_m",
    "geometry_source_current_radius_over_r0",
    "geometry_source_running_max_normalized_blunting",
    "geometry_source_capacity_ratio",
    "geometry_source_capacity_total",
    "geometry_source_cumulative_exposed",
    "developed_state_active_count",
    "developed_state_retained_count",
)
for key in required:
    assert all(
        isinstance(r[key], bool) if key == "geometry_source_feedback_armed"
        else math.isfinite(float(r[key]))
        for r in records
    ), key
fired = [r for r in records if bool(r.get("fired", False))]
assert fired, "no first advance"
first = fired[0]
assert first["geometry_source_feedback_armed"] is True, first
assert math.isclose(
    float(first["geometry_source_reference_radius_m"]),
    float(first["geometry_source_r0_m"]),
    rel_tol=0.0,
    abs_tol=1e-18,
), first
assert math.isclose(float(first["geometry_source_capacity_ratio"]), 1.0, abs_tol=1e-12), first
assert float(first["geometry_source_cumulative_exposed"]) <= 1e-12, first
if kind == "baseline":
    assert max(float(r["developed_state_cumulative_emitted"]) for r in records) <= 1e-12
    assert max(float(r["geometry_source_cumulative_exposed"]) for r in records) <= 1e-12
if gain == 0.0:
    assert max(abs(float(r["geometry_source_capacity_ratio"]) - 1.0) for r in records) <= 1e-12
    assert max(float(r["geometry_source_cumulative_exposed"]) for r in records) <= 1e-12
PY
    then
      status=FAILED
    fi
  fi

  printf '%s\t%s\t%s\t%s\t%s\n' \
    "$kind" "$gain" "$temp" "$status" "$outdir" >> "$MANIFEST"
  if [[ "$status" == FAILED ]]; then
    echo "ERROR: geometry source case failed: $kind gain=$gain T=$temp" >&2
    exit 1
  fi
}

for T_K in $TEMPS; do
  run_case baseline 0 "$T_K" \
    "$OUTROOT/baseline/T${T_K}_th${THETA}" \
    --no-tip-plasticity --no-active-shielding --signed-active-shielding
done

for GAIN in $GEOMETRY_GAINS; do
  GTAG=$(scale_tag "$GAIN")
  for T_K in $TEMPS; do
    run_case full "$GAIN" "$T_K" \
      "$OUTROOT/full/G${GTAG}/T${T_K}_th${THETA}" \
      --tip-plasticity --active-shielding --signed-active-shielding
  done
done

"$PYTHON_BIN" scripts/analyze_v10_1_9_geometry_source.py \
  --root "$OUTROOT" \
  --temperatures $TEMPS \
  --geometry-gains $GEOMETRY_GAINS \
  --theta "$THETA"

echo "wrote $MANIFEST"
