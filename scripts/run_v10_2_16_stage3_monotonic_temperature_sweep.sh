...
OUTROOT=${OUTROOT:-runs/v10_2_16_stage3_continuum_four_option_100um_theta45_v1}
...
case "$MODE" in
  full) ;;
  smoke)
    OPTIONS=${OPTIONS_SMOKE:-dbtt_primary}
    TEMPS=${TEMPS_SMOKE:-700}
    TARGET_EXT_UM=${TARGET_EXT_UM_SMOKE:-20}
    STEPS=${STEPS_SMOKE:-20000}
...
if arrhenius_fracture.PROJECT_RELEASE != "10.2.16":
    raise SystemExit(f"wrong project release: {arrhenius_fracture.PROJECT_RELEASE!r}")
...
cat <<EOF
v10.2.16 Stage 3 continuum-source plan
  mode:         $MODE
  cases:        $N_CASES
  options:      $OPTIONS
  temperatures: $TEMPS
  ...
  theta:        $THETA deg
  max jobs:     $MAX_JOBS
  source law:   continuum activity + Peierls clearing + geometric renewal
  output:       $OUTROOT
  plan:         $PLAN
EOF
...
run_case() {
  ...
  local mpz_bins=$5
  local case_root=$6
  local label="$option/T${temperature}K"
  ...
  if [[ "$SKIP_FINISHED" == "1" && -f "$case_root/stage3_case_status.json" ]]; then
    echo "SKIP: $label already classified"
    return 0
  fi

  local cmd=(
    "$PYTHON_BIN" -m arrhenius_fracture.sharp_front_v10_2_16
    --mode 2d
...

  {
    ...
    printf 'PYTHONUNBUFFERED=1 CLEAVAGE_HAZARD_MODE=deterministic '
    printf 'CLEAVAGE_EVENT_LENGTH_MODE=fixed ANISOTROPIC_TRANSPORT_MODE=validated_scalar '
  ...
  echo "START: $label candidate=$candidate mpz=${mpz_length}um/${mpz_bins}bins target=${TARGET_EXT_UM}um"
  "${cmd[@]}" 2>&1 | "$PYTHON_BIN" -u -c '
import sys
prefix = sys.argv[1]
...
' "$label" | tee "$log"
  ...
  rc=$?
  if [[ "$rc" -ne 0 ]]; then
    echo "classification_exit_$rc" > "$case_root/RUN_FAILED"
    return "$rc"
  fi
  echo "COMPLETE: $label target=${TARGET_EXT_UM}um"
}
...
echo "CAMPAIGN COMPLETE: failures=$FAILURES cases=$N_CASES output=$OUTROOT"
