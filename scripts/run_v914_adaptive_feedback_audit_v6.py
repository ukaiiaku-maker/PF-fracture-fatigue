#!/usr/bin/env python3
"""Run one-cycle adaptive feedback convergence for v5 finite-tip shielding.

This runner is deliberately event-free and audit-only.  It advances exactly one
physical cycle and reports the converged constitutive/hazard trajectory before
any stochastic fracture event.  It is intended for the weak-T R=-0.95
qualification condition where the fixed-grid one-cycle calculations contain no
fracture events but do contain a narrow reverse-transport/shielding transient.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--shield-mode",
        choices=("radius-floor-v5", "radius-shift-v5"),
        required=True,
    )
...
def main() -> int:
    ...
    # phase_steps only defines the waveform object's nominal partition.  The
    # adaptive integrator evaluates K_at_phase continuously and supplies its own
    # accepted phase intervals.
    ...
    if args.shield_mode == "radius-floor-v5":
        state_cls = FiniteTipFloorReversibleState
    else:
        state_cls = FiniteTipShiftReversibleState

    state_cls.coupled_operator_substeps = int(args.coupled_substeps)
    initial = state_cls(candidate, physics)
    ...
    final_state, hazard, telemetry = adaptive_one_cycle(
    ...
    ktr = np.asarray(
        [sample["transport_K_signed_MPa_sqrt_m"] for sample in samples],
        dtype=float,
    )
    ksh = np.asarray(
    ...
    result = {
        "schema": "v914_adaptive_feedback_audit_v6_result_v1",
    ...
    contract = {
        "schema": "v914_adaptive_feedback_audit_v6_contract_v1",
        ...
        "transport_storage_physics_changed_from_v5": False,
        "return_semantics_changed_from_v5": False,
        "barrier_physics_changed_from_v5": False,
    }
    ...
    print(
        ...
        f"Ktr_min={result['transport_K_min_MPa_sqrt_m']:.8g}",
