...
SWEEP_ROOT=${SWEEP_ROOT:-runs/v10_2_28_theta45_loading_rate_sweep_base3621_v1}
...
if not math.isclose(target, 1000.0, rel_tol=0.0, abs_tol=1e-12):
    raise SystemExit("ERROR: this sweep is fixed to 1000 um projected extension")
if max_jobs != 1:
    raise SystemExit("ERROR: theta45 rate sweep requires MAX_JOBS=1")
PY
...
payload = {
    "schema": "v10.2.28_theta45_loading_rate_sweep_v1",
...
(root / "v10_2_28_theta45_loading_rate_sweep_manifest.json").write_text(
...
echo "Theta45 loading-rate sweep complete: $SWEEP_ROOT"
