...
class Basis:
    ...
    def derivative(self, stress_Pa):
        s = np.maximum(np.asarray(stress_Pa, dtype=float), 0.0)
        q = self.value(s)
        # np.where keeps the scalar and array paths identical.  The guarded
        # denominator also avoids evaluating a 0/0 branch for scalar stress.
        safe_s = np.where(s > 0.0, s, 1.0)
        derivative = (-self.alpha * self.exponent
                      * (s / self.sigma_Pa) ** self.exponent * q / safe_s)
        return np.where(s > 0.0, derivative, 0.0)
...
class ThermodynamicSurface:
    ...
    def rate(self, stress_Pa, temperature_K):
        """Compatibility alias for analysis-only reduced operators."""
        return self.raw_rate_s(stress_Pa, temperature_K)
...
def candidate_surface_from_parameters(parent_manifest, parameters: Mapping[str, float],
    ...
    second_sigma = parameters.get("emission_second_basis_sigma_Pa")
    if second_sigma is not None and math.isfinite(float(second_sigma)) and float(second_sigma)>0:
        second = Basis(1.0, float(second_sigma), float(parameters.get("emission_second_basis_exponent", 4.0)))
        matrix = np.array([[1.0, 1.0],
                           [float(ebasis.value(emission_active_stress_Pa)),
                            float(second.value(emission_active_stress_Pa))]])
        rhs = np.array([float(parameters["emission_entropy_zero_kB"])-float(parameters["emission_entropy_infinity_kB"]),
                        float(parameters["emission_entropy_active_kB"])-float(parameters["emission_entropy_infinity_kB"])])
        if abs(np.linalg.det(matrix)) < 1e-10: raise ValueError("singular secondary emission entropy coordinates")
        e_entropy = tuple(np.linalg.solve(matrix, rhs)); ebases=(ebasis,second);eamps=(eamp,0.0)
        cp_rhs=np.array([float(parameters.get("emission_heat_capacity_zero_kB",0.0))-float(parameters.get("emission_heat_capacity_infinity_kB",0.0)),float(parameters.get("emission_heat_capacity_active_kB",0.0))-float(parameters.get("emission_heat_capacity_infinity_kB",0.0))])
        e_cp=tuple(np.linalg.solve(matrix,cp_rhs));e_cp_inf=float(parameters.get("emission_heat_capacity_infinity_kB",0.0))
    else:
        ...
        ebases=(ebasis,);eamps=(eamp,);e_cp=(parameters.get("emission_heat_capacity_base_kB", 0.0),);e_cp_inf=0.0
    ...
    emission = ThermodynamicSurface("emission", parent_manifest.emission.attempt_frequency_s,
        efloor, eamps, ebases, parameters["emission_entropy_infinity_kB"],
        e_entropy, e_cp_inf, e_cp)
