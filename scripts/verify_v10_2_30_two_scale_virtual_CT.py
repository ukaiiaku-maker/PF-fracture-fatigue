...
def main():
 ...
 mandatory=verify_jobs(root/"two_scale_anchor_job_registry.csv",9);refinement=verify_jobs(root/"A_NATIVE_low_K_refinement"/"A_NATIVE_refinement_job_registry.csv",3);PT=verify_jobs(root/"PT_overlay_anchors"/"PT_anchor_job_registry.csv",8)
 ...
 refined=pd.read_csv(root/"A_NATIVE_refinement_validation.csv");require(len(refined)==3 and refined.admissible.all(),"anchor exceeds error gate without refinement");PT_validation=pd.read_csv(root/"PT_conditional_anchor_validation.csv");require(len(PT_validation)==8 and PT_validation.admissible.all(),"conditional PT interpolation unresolved")
