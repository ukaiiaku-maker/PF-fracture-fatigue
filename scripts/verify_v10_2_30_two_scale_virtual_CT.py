...
def main():
 ...
 mandatory=verify_jobs(root/"two_scale_anchor_job_registry.csv",9);refinement=verify_jobs(root/"A_NATIVE_low_K_refinement"/"A_NATIVE_refinement_job_registry.csv",3);PT=verify_jobs(root/"PT_overlay_anchors"/"PT_anchor_job_registry.csv",20)
 ...
 refined=pd.read_csv(root/"A_NATIVE_refinement_validation.csv");require(len(refined)==3 and refined.admissible.all(),"anchor exceeds error gate without refinement")
 PT_validation=pd.read_csv(root/"PT_conditional_anchor_validation.csv");require(len(PT_validation)==20,"conditional PT validation matrix incomplete")
 PT_failures=PT_validation[~PT_validation.admissible];PT_refinements=PT_validation[PT_validation.stage=="PT_LOW_K_REFINEMENT"]
 require(len(PT_failures)==6 and (PT_failures.stage=="PT_LOW_K_13P5").all() and np.allclose(PT_failures.Kmax_MPa_sqrt_m,13.5),"unexpected conditional PT interpolation failure")
 require(len(PT_refinements)==6 and PT_refinements.admissible.all() and np.allclose(PT_refinements.Kmax_MPa_sqrt_m,12.75),"conditional PT midpoint refinement did not pass")
 pairs=PT_failures[["option","R"]].merge(PT_refinements[["option","R"]],on=["option","R"],how="inner");require(len(pairs)==6,"each PT low-K failure lacks its option/R midpoint validation")
 ...
 curves=pd.read_parquet(root/"virtual_CT_curves.parquet");summary=pd.read_csv(root/"virtual_CT_life_summary.csv");require(len(summary)==42,"virtual protocol matrix incomplete");require(len(curves)==42*401,"virtual curve row matrix incomplete");require(not any(c.lower() in {"deltak_eff","delta_k_eff","effective_deltak","effective_delta_k"} for c in curves.columns),"closure-corrected DeltaK_eff reported");require(not any("radius" in c.lower() for c in curves.columns),"tip radius entered nominal K");require((summary.K_domain_margin_low>=-1e-10).all() and (summary.K_domain_margin_high>=-1e-10).all(),"virtual path leaves validated local domain");require((summary.integration_convergence<1e-5).all(),"numerical integration convergence failed")
