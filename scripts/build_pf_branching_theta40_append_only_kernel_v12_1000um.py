#!/usr/bin/env python3
"""Build the authorized append-only theta-40 signed-kernel family to 1600 um.

The qualified V5.3 0--745 um state and load-invariance payloads are copied
without modification. Only 800, 1000, 1250, and 1600 um are newly measured.
The emitted family uses a nested v10.2.14 exact-prefix resolver so neither this
append nor the earlier 415--745 um append perturbs its qualified prefix.
"""
...
SCHEMA = "pf_current_source_general_multifront_v12_append_only_kernel_1600um"
OLD_LEVELS_UM = (0.0, 200.0, 400.0, 415.0, 420.0, 425.0, 600.0, 745.0)
NEW_LEVELS_UM = (800.0, 1000.0, 1250.0, 1600.0)
ALL_LEVELS_UM = OLD_LEVELS_UM + NEW_LEVELS_UM
OLD_STATE_IDS = (
    "E0000000", "E0000200", "E0000400", "E0000415",
    "E0000420", "E0000425", "E0000600", "E0000745",
)
NEW_STATE_IDS = ("E0000800", "E0001000", "E0001250", "E0001600")
...
def _validate_prefix_state_rows(old_payload: dict[str, Any], new_payload: dict[str, Any]) -> None:
    ...
    if old_rows != new_rows:
        raise RuntimeError("assembled family changed at least one preserved 0--745 um state")
...
def _evaluate_prefix_identity(
    ...
    quantum = np.arange(0.0, 745.0 + 2.5, 5.0)
...
def main() -> int:
    ...
    anchor_plan = {
        ...
        "required_max_extension_um": 1600.0,
    ...
    family_payload.update(
        {
            ...
            "append_only_legacy_domain_policy": {
                ...
                "legacy_domain_max_crack_extension_m": 745.0e-6,
                "legacy_state_ids": list(OLD_STATE_IDS),
                "legacy_family_sha256": EXPECTED_OLD_FAMILY_SHA256,
                "legacy_append_only_legacy_domain_policy": copy.deepcopy(
                    old_family_payload.get("append_only_legacy_domain_policy")
                ),
    ...
    coverage = {
        "schema": "pf_general_multifront_v12_signed_kernel_coverage_1600um",
        "passed": True,
        "policy": "strict_measured_envelope_no_clipping_endpoint_hold_or_extrapolation",
        "scope": "owner_local_process_coordinate_for_v12_1000um_cap6_run",
        "shared_process_query_upper_bound_um": 1595.0,
        "topology_quantum_guard_um": 5.0,
        "measured_endpoint_um": 1600.0,
