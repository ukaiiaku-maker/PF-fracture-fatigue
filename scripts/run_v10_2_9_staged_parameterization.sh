...
# Quality-diversity defaults.  One quarter of every promotion set is protected
# by pure objective rank; the remainder balances objective quality with distance
# in both material-parameter and predicted-response space.
...
case "$STAGE" in
  analytical)
    ...
    : "${ENGINE_TEMPLATE:?Set ENGINE_TEMPLATE to the complete v10.2.6 engine config JSON}"
...
python scripts/run_v10_2_9_staged_parameterization.py \
  "${args[@]}" \
