...
import numpy as np
ART=ROOT/'artifacts/row_renewal_monotonic_forward';FIG=ART/'figures'
...
def main():
 ...
 def read(name):
  ...
  return [r for r in rows if 'threshold_mode' not in r or r['threshold_mode']=='SEED_1720']
 ...
 cells=[[LABELS[r['candidate_id']],f"{float(r['K_FP_300K']):.4g}",f"{float(r['K_FP_1200K']):.4g}",('Accessible low-T branch' if r['candidate_id']=='A_NATIVE' else 'Zero-load dominated from 300 K'),'F2 unavailable'] for r in cross]
 ...
 fig,ax,p=figure('11_GENERIC_VS_ROW_RENEWAL','Row-specific / generic KFP — unchanged barriers and gates')
 comp=read('generic_vs_row_renewal_comparison.csv')
 for cid,color in COLORS.items():
  rr=[r for r in comp if r['candidate_id']==cid and r['tier']=='F0_INTRINSIC_OPENING' and float(r['Kdot'])==.005]
  line(ax,p,'generic_vs_row_renewal_comparison.csv',rr,'temperature_K','K_FP_ratio',LABELS[cid],color=color)
 finish(fig,ax,p,'Temperature (K)','KFP row-specific / KFP generic',True)
 fig,ax,p=figure('12_THRESHOLD_ROBUSTNESS','Absolute first-passage loads across all three thresholds')
 robust=list(csv.DictReader((ART/'threshold_robustness.csv').open()))
 for cid,color in COLORS.items():
  rr=sorted([r for r in robust if r['candidate_id']==cid and float(r['Kdot'])==.005],key=lambda r:float(r['threshold_action']))
  for col,ls in [('K_FP_300K','-'),('K_FP_1200K','--')]:
   line(ax,p,'threshold_robustness.csv',rr,'threshold_action',col,LABELS[cid]+' '+col.replace('K_FP_',''),color=color,ls=ls,marker='o')
 ax.set_xscale('log');finish(fig,ax,p,'Threshold action Xi','KFP (MPa √m)',True)
