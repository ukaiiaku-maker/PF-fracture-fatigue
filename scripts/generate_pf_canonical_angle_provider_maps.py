...
def state(mesh, boundary, damage, D, material, tip, tangent, segments, kill_radius):
    ...
    u, reaction = solve_dirichlet(K, R0, u0, boundary, .5 * REFERENCE_OPENING_M, -.5 * REFERENCE_OPENING_M)
    ...
    return u, sigma, {
        ...
        "compliance_m2_per_N": float(REFERENCE_OPENING_M / reaction),
...
def generate(theta_deg: float, target_projected_um: float, out: Path) -> dict[str, Any]:
    ...
    for count in range(event_count + 1):
        ...
        for radius_um in RADII_UM:
            ...
            opening_stress = float(np.asarray(probe["opening_tensor_Pa"], dtype=float)[1, 1])
            drives.append({
