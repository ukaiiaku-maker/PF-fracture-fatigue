...
def test_active_cleavage_shielding_uses_uncapped_shared_core():
    ...
    assert eng.mpz._campaign_max_K_shield_Pa_sqrt_m == pytest.approx(0.0)
    assert eng.mpz._campaign_legacy_max_K_shield_reference_Pa_sqrt_m == pytest.approx(
        legacy_reference
    )
    assert diag["campaign_active_K_shield_cap_Pa_sqrt_m"] == pytest.approx(0.0)
    assert diag["campaign_legacy_K_shield_cap_reference_Pa_sqrt_m"] == pytest.approx(
        legacy_reference
    )
    assert diag["campaign_shielding_cap_applied"] is False
    assert diag["campaign_shielding_population_limited"] is True
