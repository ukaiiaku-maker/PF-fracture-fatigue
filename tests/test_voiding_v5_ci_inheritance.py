...
def test_machine_readable_general_ci_inheritance_is_exact():
    ...
    ownership = set(record["ownership_topology_head"]["failure_ids"])
    assert len(base) == len(head) == len(hardened) == len(ownership) == 7
    assert base == head == hardened == ownership
    ...
    assert record["ownership_topology_head"]["git_sha"] == "a82a265af4dcfae41aad50dcc98531edb58d9310"
    assert record["ownership_topology_head"]["run_ids"] == [33922841723, 33922845369]
