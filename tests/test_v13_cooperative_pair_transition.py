...
def test_competing_pair_probability_and_exact_fallback():
    ...
    parent=NS(front_lineage=('a',),primary_event_id='event:1',primary_event_ordinal=1,primary_candidate_id='a',
        accepted_state_id='accepted:1',process_state_sha256='process:1')
    state=NS(competition=NS(consumed_event_ids=('event:1',)),rng_state=object(),tip_process_state=object(),event_counters=object())
    identities={'observation_accepted_state_id':'accepted:1','observation_process_sha256':'process:1'}
    ...
    veto=apply_cooperative_pair_transition(parent=parent,baseline_single_state=state,companion_id='b',
        parameters=p,temperature_K=300,rate_balance=1,**identities,
    ...
    with pytest.raises(ValueError,match='identity'):
        apply_cooperative_pair_transition(parent=parent,baseline_single_state=state,companion_id='b',
            ...
            exact_pair_trial=lambda *_:pytest.fail('stale observation reached pair provider'))
