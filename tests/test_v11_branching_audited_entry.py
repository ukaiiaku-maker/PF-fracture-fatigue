...
def test_audited_entry_rejects_conflicting_forced_modes():
    ...
    with pytest.raises(SystemExit, match="maximum-fronts=16"):
...
def test_audit_only_writes_complete_fail_closed_policy(tmp_path):
    assert main([
        ...
        "--maximum-fronts", "16", "--out", str(tmp_path), "--audit-only",
    ...
    assert payload["policy"]["maximum_fronts"] == 16
    assert payload["policy"]["maximum_branch_births"] == 8
