...
def test_exact_element_exclusion_is_optional_validated_and_zeroes_contribution():
    ...
    with pytest.raises(ValueError, match="Boolean array with one entry per element"):
        compute_signed_interaction_integral(
            mesh, u, sigma, damage,
    ...
    with pytest.raises(ValueError, match="Boolean array with one entry per element"):
        compute_signed_interaction_integral(
            mesh, u, sigma, damage,
            exclude_element_mask=np.zeros(mesh.ne, dtype=int), **kwargs
        )
    ...
    zero_area = compute_signed_interaction_integral(zero_area_mesh, u, sigma, damage, **kwargs)
