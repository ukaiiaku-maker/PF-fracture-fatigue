...
def _anisotropic_continuum_emit(self, dt: float, stress_Pa: float, T_K: float,
                                 system_weights: np.ndarray | None = None) -> float:
    """Evolve aggregate continuum source activity for each anisotropic channel."""
    dt = max(float(dt), 0.0)
    if dt <= 0.0:
        return 0.0
    ...
    transport = self._transport_rates(stress_profile, rho_transport, T_K, self._continuum_b)
    velocity_clear = float(np.mean(np.maximum(np.asarray(transport["velocity"][:nsrc]), 0.0)))
    radius = max(
    ...
    rates_per_site = np.maximum(
        np.asarray([self.emission_rate_per_site(float(value), T_K) for value in sigma_emit], dtype=float),
        0.0,
    )
    ...
    equilibrium = np.divide(clear_rate, total_rate, out=np.ones_like(total_rate), where=total_rate > 0.0)
    ...
    activity1 = np.clip(equilibrium + (activity0 - equilibrium) * decay, 0.0, 1.0)
    ...
    self.continuum_source_last_effective_multiplicity = float(np.sum(reference * activity1))
    self.continuum_source_last_emission_rate_s = float(np.sum(aggregate_hazard * activity0))
    ...
    self.campaign_source_budget_remaining_total = float(self.n_systems * reference)
...
def install_anisotropic_continuum_emission(state: Any,
                                             config: _anisotropic.AnisotropicEmissionConfig) -> None:
...
__all__ = [
    "MODEL_ID", "SOURCE_MODEL", "_anisotropic_continuum_emit",
    "install_anisotropic_continuum_emission", "audit_payload",
]
