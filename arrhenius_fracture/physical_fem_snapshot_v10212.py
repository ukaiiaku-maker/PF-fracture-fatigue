"""Serializable fixed-crack FEM snapshots for real signed-kernel generation."""
...
MODEL_ID = "v10.2.12_serialized_physical_fixed_crack_fem_state"
RESPONSE_COLUMNS = (
    "state_id",
    "r_eff_over_r0",
...
class SnapshotMetadata:
    ...
    def validate(self) -> "SnapshotMetadata":
        ...
        if self.cohesive_network_present:
            raise ValueError(
                "v10.2.12 snapshot replay does not serialize cohesive-network state; "
...
def load_snapshot(root: str | Path) -> dict[str, Any]:
    ...
    if payload.get("schema") != MODEL_ID:
    ...
    for key in (
        ...
        if key in metadata_fields:
            ...
            if key in {"channel_directions", "channel_normals"}:
...
def generate_signed_responses(
    ...
    *,
    ...
    for region, coordinates in (("active", meta.active_x_m), ("wake", meta.wake_x_m)):
        for system, (slip, normal) in enumerate(zip(channel_directions, channel_normals)):
            ...
            for bin_index, x_m in enumerate(coordinates):
                distance = max(float(x_m), 0.5 * width)
                if region == "active":
                    start = tip.copy()
                    end = tip + distance * oriented_slip
                else:
                    start = tip.copy()
                    end = tip - distance * forward
                for sign in (-1, 1):
                    for magnitude in values:
                        ...
                        perturbation_audit = response["perturbation"]
                        rows.append(
                            {
                                ...
                                "mesh_area_ratio": perturbation_audit["mesh_area_ratio"],
                            }
                        )
    ...
    report = {
        ...
        "n_systems": len(channel_directions),
        "active_bins": len(meta.active_x_m),
        "wake_bins": len(meta.wake_x_m),
