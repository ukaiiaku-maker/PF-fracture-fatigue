...
def _validate_measured_group(
    ...
    *,
    ...
    for mode in ("I", "II"):
        ...
        sign_means = {
            str(sign): float(np.mean([_coefficient(row, mode) for row in by_sign[sign]]))
            for sign in (-1, 1)
        }
...
def _leave_one_out_error(x: np.ndarray, y: np.ndarray) -> dict[str, Any]:
    ...
    for index in range(1, x.size - 1):
        ...
        error = abs(predicted - float(y[index])) / max(abs(float(y[index])), scale_floor)
        errors.append(
            {
    ...
    return {
        "available": True,
        "maximum_relative_error": max((row["relative_error"] for row in errors), default=0.0),
        "checks": errors,
    }
...
def expand_station_response_files(
    ...
    *,
    ...
    for raw_path in paths:
        ...
        physical_inputs.append({"path": str(path.resolve()), "audit": audit})
    ...
    measured_coefficients: dict[tuple[str, str, int], list[tuple[int, float, float, float]]] = defaultdict(list)
    linearity = []
    ...
    for key, rows in grouped.items():
        ...
        for row in rows:
            base_values[(state_id, "I")].append(_finite(row, "K_I_base_Pa_sqrt_m"))
            base_values[(state_id, "II")].append(_finite(row, "K_II_base_Pa_sqrt_m"))
            magnitudes_by_state[state_id].add(abs(_finite(row, "delta_signed_line_content")))
    ...
    for state_id, audit in state_audits.items():
        ...
        for region, full_x in grids.items():
            ...
            for system in systems:
                stations = sorted(measured_coefficients[(state_id, region, system)])
                if len(stations) < 2:
                    raise ValueError(
                        f"state {state_id} {region} system {system} requires at least two measured stations"
                    )
                ...
                if bins[0] != 0 or bins[-1] != full_x.size - 1:
                    raise ValueError(
                        f"state {state_id} {region} system {system} lacks endpoint station coverage"
                    )
                if np.any(np.diff(x) <= 0.0):
                    raise ValueError("measured station coordinates must be strictly increasing")
                            ...
                            expanded_rows.append(
                                {
                                    ...
                                    "K_I_perturbed_Pa_sqrt_m": base_I - float(projected_I[bin_index]) * content,
                                    "K_II_base_Pa_sqrt_m": base_II,
                                    "K_II_perturbed_Pa_sqrt_m": base_II - float(projected_II[bin_index]) * content,
                                    "interaction_integral_schema": rows[0]["interaction_integral_schema"],
                                    "ribbon_width_m": audit["ribbon_width_m"],
                                    "mesh_area_ratio": 1.0,
                                }
                            )
    ...
    report = {
        ...
        "spatial_cross_validation_tolerance": float(spatial_cross_validation_tolerance),
