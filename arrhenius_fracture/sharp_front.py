...
def build_engine(args, mat) -> FrontEngine:
    bundle = getattr(args, "unified_material_bundle", None)
    if bundle is not None:
        from .unified_fracture_material_v5 import canonical_front_engine
        return canonical_front_engine(bundle, mat)
    if bool(getattr(args, "unified_scientific_execution", False)):
        raise ValueError("NO_DEFAULT_FALLBACK: unified scientific execution requires a material bundle")
    # The remaining constructor is retained for explicitly legacy/benchmark
    # entry points. Unified single-tip, multi-tip and voiding controls never use it.
    f = FrontConfig()
    f.r0 = args.r_pz
