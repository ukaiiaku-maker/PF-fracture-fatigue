"""v10.2.13 extension-only real signed atlas entry for fracture and fatigue."""
...
from .state_resolved_signed_engine_v10213 import (
    MODEL_ID,
...
def _write_audit(args: list[str], family, transport_mode: str) -> None:
    ...
    (root / "v10_2_13_extension_only_signed_shared_physics.json").write_text(
...
def main(argv=None):
    ...
    if not family_path:
        raise SystemExit(
            "v10.2.13 requires --signed-kernel-family PATH or "
            "SIGNED_KERNEL_FAMILY_JSON. Static, unsigned, or opening-interpolated "
            "fallback operators are not permitted."
        )
    family = ExtensionOnlySigned2DShieldingKernelFamily.from_json(family_path)
    if not bool(family.metadata.get("production_parameterization_allowed", False)):
        if _bool_env("PARAMETER_CAMPAIGN", False):
            raise SystemExit(
                "v10.2.13 atlas has not authorized production parameterization; "
                "complete frozen-geometry load invariance, mesh/ribbon/station "
                "convergence, normalization, replay, and full 2-D fracture/fatigue gates"
            )
