...
def _write_v10216_audit(
    ...
    # Replace inaccurate legacy source-language in the inherited run audit while
    # retaining all original diagnostics and transport records.
    ...
    transport = (
        json.loads(transport_path.read_text()) if transport_path.exists() else {}
    )
    transport.update(
        {
            "schema": MODEL_ID,
