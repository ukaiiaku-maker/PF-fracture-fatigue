...
def compute_signed_interaction_integral(
    ...
    *,
    ...
    if exclude_element_mask is not None:
        exclude_element_mask=np.asarray(exclude_element_mask,bool)
        if exclude_element_mask.shape!=(mesh.ne,):
            raise ValueError("exclude_element_mask must have one entry per element")
