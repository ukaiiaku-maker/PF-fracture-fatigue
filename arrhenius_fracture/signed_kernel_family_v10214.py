...
class ActiveOnlySigned2DShieldingKernelFamily(_V10212Family):
    ...
    def _install_append_only_legacy_domain(self) -> None:
        ...
        nested_policy = policy.get("legacy_append_only_legacy_domain_policy")
        if nested_policy is not None and not isinstance(nested_policy, dict):
            raise ValueError("nested append-only legacy-domain policy must be an object")
        legacy_metadata = {
        ...
        if nested_policy is not None:
            legacy_metadata["append_only_legacy_domain_policy"] = copy.deepcopy(
                nested_policy
            )
        legacy = type(self)(
            states=[
        ...
        legacy._install_append_only_legacy_domain()
