"""v10.2.9 effective-opening signed-kernel entry point for fracture and fatigue."""
...
def _write_audit(args: list[str], family, transport_mode: str) -> None:
    ...
    (root / "v10_2_9_effective_opening_signed_physics.json").write_text(
...
def main(argv=None):
    ...
    if not family_path:
        raise SystemExit(
            "v10.2.9 requires --signed-kernel-family PATH or "
            "SIGNED_KERNEL_FAMILY_JSON. Static, unsigned, or unreviewed fallback "
    ...
    if not bool(family.metadata.get("production_parameterization_allowed", False)):
        if _bool_env("PARAMETER_CAMPAIGN", False):
            raise SystemExit(
                "v10.2.9 kernel family has not authorized production "
                "parameterization; complete analytic interaction-integral, state-grid, "
                "normalization, boundary-stationarity, replay, and 2-D validation gates"
            )
    ...
    try:
        ...
        print(
            "  v10.2.9 effective-opening signed physics: "
            f"loading={loading} transport={transport_mode} states={len(family.states)} "
            f"interpolation={family.interpolation.get('method')} Kcap=off"
        )
